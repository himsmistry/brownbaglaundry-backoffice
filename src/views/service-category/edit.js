...
class edit extends Component {
	...
	submit() {
		if(this.validation()){
			...
			if (this.state.fields['icon'] === 'icon' || this.state.fields['icon']===''){
			...
			fetch(configuration.baseURL+"service-category/update", {
				method: "post",
				headers: {
	...
	componentWillMount(){
        ...
        service_category_id=url.substring(url.lastIndexOf('/') + 1);
		
		fetch(configuration.baseURL+"service-category/get?service_category_id="+service_category_id).then((response) =>{
	    	return response.json();
	    }).then((data)=> {
	            			...
	            			<CForm className="form-horizontal">
				                ...
				                <CFormGroup row>
				                	<CCol md="6">
				                    	...
				                    	{	this.state.fields.icon==='' || this.state.fields.icon==='icon'?
					                    	<input
	                                        	accept="image/*"
	                                            type="file"
	                                        ...
	                                        <div>
		                                        <CImg
								                    ...
								                    src={this.state.fields['icon']}
								                    className="c-avatar-img"
								                    alt="laundry"
								                /><br/>
								                <CButton color="danger" className="px-4" onClick={this.handleRemovePhoto.bind(this, this.state.fields['icon'],'icon')}>Remove</CButton>
							                </div>
	                                    }
