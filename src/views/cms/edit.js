...
class edit extends Component {
	...
	submit() {
		if(this.validation()){
			...
			fetch(configuration.baseURL+"cms/update", {
				method: "post",
				headers: {
	...
	componentWillMount(){
        ...
        cms_id=url.substring(url.lastIndexOf('/') + 1);
		
		fetch(configuration.baseURL+"cms/get?cms_id="+cms_id).then((response) =>{
	    	return response.json();
	    }).then((data)=> {
	            			...
	            			<CForm className="form-horizontal">
				                ...
				                <CFormGroup row>
				                	<CCol md="6">
				                    	...
				                    	<CSelect custom onChange={this.handleChange.bind(this,'type')} value={this.state.fields.type} disabled={true}>
