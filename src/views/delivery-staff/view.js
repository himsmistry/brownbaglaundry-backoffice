...
class view extends Component {
    ...
    componentDidMount(){
		fetch(configuration.baseURL+"delivery-staff/get").then((response) => {
	...
	handleAction(type) {
		...
		fetch(configuration.baseURL+"delivery-staff/action", {
			method: "post",
			headers: {
	...
	selectall(){
		if(this.state.checkAllSelect === false){
			...
			_.each(this.state.data, (element) => {
				selected.push(element.delivery_staff_id);
			});
    ...
    render() {
    	const fields = [
		    ...
		    { key: 'delivery_staff_id',label:'#ID', _style: { width: '15%'} },
		    { key: 'name', _style: { width: '20%'} },
		    { key: 'email', _style: { width: '20%'} },
		    { key: 'mobile', _style: { width: '20%'} },
		    { key: 'pin',label:'PIN', _style: { width: '10%'} },
	  		...
	  		<CRow>
        		...
        		<CCol xs="12" md="12">
          			<CCard>
	            		<CCardBody>
	            			<p>
	            				<CButton color="primary" className="px-4" onClick={() => this.props.history.push('/delivery-staff/add') }>Add</CButton>{' '}
						    ...
						    <CDataTable
					          	...
					          	(item)=>(
						            <td>
						              	<CInput type="checkbox" style={{width:'20px', height:'20px'}} onClick={this.checkbox.bind(this,item.delivery_staff_id)} checked={this.state.selected.includes(item.delivery_staff_id)===true}/>
						            </td>
					          	),
						      	'delivery_staff_id':
					          	(item)=>(
						            <td>
						              	<Link to={{ pathname: "/delivery-staff/edit/"+item.delivery_staff_id }} > {item.delivery_staff_id}</Link>
						            </td>
					          	),
					          	'status':
					          	(item)=>(
