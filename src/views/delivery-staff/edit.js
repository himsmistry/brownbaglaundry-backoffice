...
class edit extends Component {
    ...
    handleCancel(){
    	this.props.history.push('/delivery-staff/view')
    }
	...
	submit() {
		if(this.validation()){
			...
			fetch(configuration.baseURL+"delivery-staff/update", {
				method: "post",
				headers: {
	...
	componentWillMount(){
        ...
        delivery_staff_id=url.substring(url.lastIndexOf('/') + 1);
		
		fetch(configuration.baseURL+"delivery-staff/get?delivery_staff_id="+delivery_staff_id).then((response) =>{
	    	return response.json();
	    }).then((data)=> {
