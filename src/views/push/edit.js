...
class edit extends Component {
	...
	submit() {
		if(this.validation()){
			...
			fetch(configuration.baseURL+"push/update", {
				method: "post",
				headers: {
	...
	componentWillMount(){
        ...
        push_template_id=url.substring(url.lastIndexOf('/') + 1);
		
		fetch(configuration.baseURL+"push/get?push_template_id="+push_template_id).then((response) =>{
	    	return response.json();
	    }).then((data)=> {
