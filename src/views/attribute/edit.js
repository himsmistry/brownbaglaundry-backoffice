...
class edit extends Component {
	...
	submit() {
		if(this.validation()){
			...
			fetch(configuration.baseURL+"attribute/update", {
				method: "post",
                headers: {
	...
	componentWillMount(){
        ...
        attribute_id=url.substring(url.lastIndexOf('/') + 1);
		
		fetch(configuration.baseURL+"attribute/get?attribute_id="+attribute_id).then((response) =>{
	    	return response.json();
	    }).then((data)=> {
	            			...
	            			<CForm className="form-horizontal">
	            				<CFormGroup row>
                                    <CCol md="6">
                                        ...
                                        <CSelect custom onChange={this.handleChange.bind(this,'service_category_id')} value={this.state.fields.service_category_id} disabled={true}>
