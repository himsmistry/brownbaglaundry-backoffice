...
class edit extends Component {
	...
	submit() {
		if(this.validation()){
			...
			fetch(configuration.baseURL+"coupon/update", {
				method: "post",
				headers: {
	...
	componentDidMount(){
        ...
        coupon_id=url.substring(url.lastIndexOf('/') + 1);
		
		fetch(configuration.baseURL+"coupon/get?coupon_id="+coupon_id).then((response) =>{
	    	return response.json();
	    }).then((data)=> {
	            			...
	            			<CForm className="form-horizontal">
				                <CFormGroup row>
				                  	...
				                  	<CCol md="12">
				                    	...
				                    	<CInput type="text" placeholder="Enter code" onChange={this.handleChange.bind(this,'coupon_code')} value={this.state.fields.coupon_code} disabled/>
