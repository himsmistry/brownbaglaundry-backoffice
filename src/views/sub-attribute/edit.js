...
class edit extends Component {
	...
	submit() {
		if(this.validation()){
			...
			fetch(configuration.baseURL+"sub-attribute/update", {
				method: "post",
                headers: {
	...
	componentWillMount(){
        ...
        sub_attribute_id=url.substring(url.lastIndexOf('/') + 1);
		
		fetch(configuration.baseURL+"sub-attribute/get?sub_attribute_id="+sub_attribute_id).then((response) =>{
	    	return response.json();
	    }).then((data)=> {
                            ...
                            <CForm className="form-horizontal">
                                <CFormGroup row>
                                    <CCol md="6">
                                        ...
                                        <CSelect custom onChange={this.handleChange.bind(this,'attribute_id')} value={this.state.fields.attribute_id} disabled>
