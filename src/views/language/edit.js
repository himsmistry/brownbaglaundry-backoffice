...
class edit extends Component {
	...
	submit() {
		if(this.validation()){
			...
			fetch(configuration.baseURL+"language/update", {
				method: "post",
				headers: {
	...
	componentWillMount(){
        ...
        language_id=url.substring(url.lastIndexOf('/') + 1);
		
		fetch(configuration.baseURL+"language/get?language_id="+language_id).then((response) =>{
	    	return response.json();
	    }).then((data)=> {
	   		this.setState({ fields: data.payload[0]});
	    }); 
	}
