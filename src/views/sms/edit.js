...
class edit extends Component {
    ...
    handleCancel(){
    	this.props.history.push('/sms/view')
    }
	...
	submit() {
		if(this.validation()){
			...
			fetch(configuration.baseURL+"sms/update", {
				method: "post",
				headers: {
	...
	componentWillMount(){
        ...
        sms_template_id=url.substring(url.lastIndexOf('/') + 1);
		
		fetch(configuration.baseURL+"sms/get?sms_template_id="+sms_template_id).then((response) =>{
	    	return response.json();
	    }).then((data)=> {
