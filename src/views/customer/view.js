...
class view extends Component {
    ...
    componentDidMount(){
		fetch(configuration.baseURL+"customer/get").then((response) => {
	...
	handleAction(type) {
		...
		fetch(configuration.baseURL+"customer/action", {
			method: "post",
			headers: {
	...
	selectall(){
		if(this.state.checkAllSelect === false){
			...
			_.each(this.state.data, (element) => {
				selected.push(element.customer_id);
			});
    ...
    render() {
    	const fields = [
		    ...
		    { key: 'customer_id',label:'#ID', _style: { width: '15%'} },
		    { key: 'name', _style: { width: '17%'} },
		    { key: 'email', _style: { width: '20%'} },
		    { key: 'mobile', _style: { width: '15%'} },
		    { key: 'register_type', _style: { width: '10%'} },
		    { key: 'otp', label:'OTP',_style: { width: '8%'} },
	  		...
	  		<CRow>
        		...
        		<CCol xs="12" md="12">
          			<CCard>
	            		<CCardBody>
	            			<p>
	            				<CButton color="primary" className="px-4" onClick={() => this.props.history.push('/customer/add') }>Add</CButton>{' '}
						    ...
						    <CDataTable
					          	...
					          	(item)=>(
						            <td>
						              	<CInput type="checkbox" style={{width:'20px', height:'20px'}} onClick={this.checkbox.bind(this,item.customer_id)} checked={this.state.selected.includes(item.customer_id)===true}/>
						            </td>
					          	),
						      	'customer_id':
					          	(item)=>(
						            <td>
						              	<Link to={{ pathname: "/customer/edit/"+item.customer_id }} > {item.customer_id}</Link>
						            </td>
					          	),
					          	'name':
					          	(item)=>(
						            <td>
						              	<Link to={{ pathname: "/customer/profile/"+item.customer_id }} > {item.name}</Link>
						            </td>
					          	),
					          	'status':
					          	(item)=>(
