...
class edit extends Component {
	...
	submit() {
		if(this.validation()){
			...
			fetch(configuration.baseURL+"service-addon/update", {
				method: "post",
				headers: {
	...
	componentWillMount(){
        ...
        service_addon_id=url.substring(url.lastIndexOf('/') + 1);
		
		fetch(configuration.baseURL+"service-addon/get?service_addon_id="+service_addon_id).then((response) =>{
	    	return response.json();
	    }).then((data)=> {
